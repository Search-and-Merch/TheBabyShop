...
export default function SearchPage() {
  ...
  useEffect(() => {
    ...
    if (!ORGANISATION_ID || !SEARCH_ID) {
      setError('VITE_ORGANISATION_ID and VITE_SEARCH_ID must be set in .env')
      setLoading(false)
      return
    }
  ...
  return (
    <div className="container mx-auto px-4 py-8">
      ...
      <div className="flex max-sm:flex-col gap-8 items-start">
        {query && <FacetSidebar facets={facets} selected={selectedFacets} onChange={handleFacetChange} />}
          ...
          {loading ? (
            ...
            <div className="text-center py-20 text-gray-400">
              ...
              <p className="text-base">
                {query ? 'No products found for your search.' : 'Enter a search term to get started.'}
              </p>
            </div>
          ) : (
            <>
              <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-4">
                {products.map((p, i) => (
                  <ProductCard key={p.id} product={p as unknown as FeedListProduct} index={i} />
                ))}
              </div>
